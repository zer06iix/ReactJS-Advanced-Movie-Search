...
import SeriesCastItem from './SeriesCastItem';

export default function SeriesCastScroller() {
    const { credits } = useSeriesStore();
    ...
    useEffect(() => {
        if (wrapperRef.current && containerRef.current) {
            ...
            setTranslateX(Math.max(Math.min(translateX, 0), -maxX));
        }
    }, [credits, translateX]);
            ...
            >
                {credits && credits.cast && credits.cast.length > 0
                    ? credits.cast.map((member) => (
                        <SeriesCastItem member={member} key={member.id} />
                    ))
                    : null}
            </div>
            <div
